...
export const MAHARASHTRA_REGULATORY_KNOWLEDGE: RegulatoryKnowledgeItem[] = [
  {
    ...
    whyRequired: 'Mandatory environmental clearance to ensure industrial discharge (liquid effluents and gaseous stack emissions) complies with Maharashtra pollution prevention standards before civil construction or commencement of production.',
    requiredDocuments: [
  ...
  },
  {
    ...
    keywords: ['factory licence', 'dish', 'factories act', 'form 1', 'worker safety', 'machinery', 'safety audit', 'workers', 'occupational health'],
  ...
  },
  {
    ...
    keywords: ['fire noc', 'fire safety', 'sprinkler', 'hydrant', 'form a', 'form b', 'fire audit', 'midc fire', 'fire fighting'],
  ...
  },
  {
    ...
    keywords: ['subsidy', 'incentive', 'psi 2019', 'capital subsidy', 'sgst refund', 'electricity duty exemption', 'interest subsidy', 'zone c', 'zone d', 'cmegp'],
  ...
  },
  {
    ...
    keywords: ['fssai', 'food licence', 'food safety', 'fda', 'food processing', 'spices', 'edible oil', 'hygiene', 'fostac'],
...
/**
 * Intelligent Regulatory RAG Engine
 * Resolves user questions across Why Required, Required Documents, Step-by-Step Process, Timelines & Fees, Subsidies, and Query Explanations.
 */
export function queryRegulatoryRAG(
  ...
  // Detect inquiry intention
  const isWhy = qLower.includes('why') || qLower.includes('reason') || qLower.includes('कशासाठी') || qLower.includes('का') || qLower.includes('क्यों');
  const isDocs = qLower.includes('document') || qLower.includes('papers') || qLower.includes('कागदपत्रे') || qLower.includes('दस्तावेज') || qLower.includes('checklist');
  ...
  const isFeeTimeline = qLower.includes('time') || qLower.includes('fee') || qLower.includes('cost') || qLower.includes('दिवस') || qLower.includes('शुल्क') || qLower.includes('फी') || qLower.includes('कितने');
  const isIncentive = qLower.includes('incentive') || qLower.includes('subsidy') || qLower.includes('psi') || qLower.includes('अनुदान') || qLower.includes('योजना') || qLower.includes('छूट');

  // Match items based on score
...
/**
 * Explains complicated officer technical queries in simple plain language for entrepreneurs
 */
export function explainOfficerQuery(queryText: string, approvalName: string) {
  const qLower = queryText.toLowerCase();

  let plainExplanation = '';
  let actionSteps: string[] = [];

  if (qLower.includes('etp') || qLower.includes('effluent') || qLower.includes('capacity')) {
    plainExplanation = 'The reviewing pollution officer needs verified proof that your Effluent Treatment Plant (ETP) can handle all the daily industrial washwater produced by your unit without overflowing.';
    actionSteps = [
      'Request your ETP environmental consultant to revise the hydraulic flow drawing.',
      'Ensure the drawing clearly states daily treatment capacity in KLD (Kilolitres per Day).',
      'Upload the revised stamped PDF drawing in your Application Details tab.'
    ];
  } else if (qLower.includes('fire') || qLower.includes('driveway') || qLower.includes('hydrant')) {
    plainExplanation = 'The fire safety officer noted that the peripheral road around your factory shed must be at least 6.0 meters wide for fire engine access, or static water tank capacity requires clarification.';
    actionSteps = [
      'Have your architect mark the 6.0m clear driveway on the site plan CAD drawing.',
      'Include the underground fire tank capacity calculation sheet (min 50,000 Litres).',
      'Submit the corrected blueprint response before the due date.'
    ];
  } else if (qLower.includes('structural') || qLower.includes('stability')) {
    plainExplanation = 'The DISH factory inspector needs an official Structural Stability Certificate signed by a chartered civil engineer confirming the shed can safely support your heavy machinery vibration and overhead crane load.';
    actionSteps = [
      'Contact a licensed Chartered Structural Engineer for site inspection.',
      'Obtain Form 1-A Structural Stability Certificate.',
      'Upload the signed certificate in response to the query.'
    ];
  } else {
    plainExplanation = `The department officer reviewing your ${approvalName} has requested additional clarification or supporting documentation regarding: "${queryText}".`;
    actionSteps = [
      'Review the specific clause highlighted in the query.',
      'Attach the requested clarifying document or revised plan.',
      'Submit your written explanation to resume department SLA processing.'
    ];
  }
