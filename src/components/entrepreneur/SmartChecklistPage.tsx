...
export const SmartChecklistPage: React.FC = () => {
      ...
      <div className="bg-white dark:bg-slate-900 rounded-3xl p-8 sm:p-12 border border-slate-200 dark:border-slate-800 shadow-xl text-center space-y-6 max-w-3xl mx-auto my-8 animate-in fade-in duration-200">
        ...
        <div className="space-y-2">
          ...
          <p className="text-slate-600 dark:text-slate-400 text-sm max-w-lg mx-auto">
            You must create your business project profile first so PermitFlow Nexus can calculate your customized single-window clearance requirements and enable approval applications.
          </p>
        </div>
        <button
  ...
  const getPhaseBadge = (id: string) => {
    if (id === 'appr-1' || id === 'appr-1-llp' || id === 'appr-2' || id === 'appr-3' || id === 'appr-11' || id === 'appr-4') {
      return { step: 'Phase 1', name: 'Legal & Tax Setup', badge: 'bg-blue-50 dark:bg-blue-950/40 text-blue-700 dark:text-blue-300 border-blue-200 dark:border-blue-800' };
    }
    if (id === 'appr-5') {
      return { step: 'Phase 2', name: 'Building & Infrastructure', badge: 'bg-[#F0FAF3] dark:bg-[#1E3326] text-[#2E6F40] dark:text-[#68BA7F] border-[#D4EEDC] dark:border-[#253D2C]' };
    }
    if (id === 'appr-6' || id === 'appr-8') {
      return { step: 'Phase 3', name: 'Pre-Establishment NOC Clearances', badge: 'bg-amber-50 dark:bg-amber-950/40 text-amber-800 dark:text-amber-300 border-amber-200 dark:border-amber-800' };
    }
    if (id === 'appr-7' || id === 'appr-10' || id === 'appr-14' || id === 'appr-15') {
      return { step: 'Phase 4', name: 'Factory Setup & Utilities', badge: 'bg-purple-50 dark:bg-purple-950/40 text-purple-800 dark:text-purple-300 border-purple-200 dark:border-purple-800' };
    }
    return { step: 'Phase 5', name: 'Industry-Specific Operating Clearances', badge: 'bg-emerald-50 dark:bg-emerald-950/40 text-emerald-800 dark:text-emerald-300 border-emerald-200 dark:border-emerald-800' };
  };
      ...
      <div className="bg-white dark:bg-slate-900 p-5 rounded-3xl border border-slate-200 dark:border-slate-800 shadow-xs space-y-3">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 pt-1">
          {userProjects.map((proj) => {
            ...
            const projChecklist = generateSmartChecklist(proj, applications);
            ...
            return (
              <button
                ...
                className={`p-4 rounded-2xl border text-left transition-all cursor-pointer flex flex-col justify-between space-y-3 ${
                  isSelected
                    ? 'bg-[#F0FAF3] dark:bg-[#1E3326] border-[#2E6F40] dark:border-[#68BA7F] shadow-md ring-2 ring-[#2E6F40]/30 scale-[1.01]'
                    : 'bg-white dark:bg-slate-800/40 border-slate-200 dark:border-slate-700 hover:border-[#68BA7F] hover:bg-slate-50 dark:hover:bg-slate-800 shadow-xs'
                }`}
              >
                <div className="space-y-1">
                  <div className="flex items-center justify-between gap-1">
                    <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-extrabold uppercase ${
                      isSelected ? 'bg-[#2E6F40] text-white' : 'bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-300'
                    }`}>
                      {proj.sector || 'Manufacturing'}
                    </span>
                    {isSelected && (
                      <span className="text-[10px] font-extrabold text-[#2E6F40] dark:text-[#68BA7F] flex items-center gap-1">
                        ● Active Checklist
                      </span>
                    )}
                  </div>
                  <h3 className="font-extrabold text-xs text-slate-900 dark:text-white truncate pt-1">
                    {proj.businessName}
                  </h3>
                  <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium truncate">
                    {proj.district} • {proj.entityType}
                  </p>
                </div>

                <div className="pt-2 border-t border-slate-100 dark:border-slate-700/60 flex items-center justify-between text-[10px] font-bold">
                  <span className="text-slate-600 dark:text-slate-400">{projChecklist.length} Clearances</span>
                  <span className="text-[#2E6F40] dark:text-[#68BA7F]">{approvedCount}/{projChecklist.length} Approved</span>
                </div>
              </button>
            );
          })}
