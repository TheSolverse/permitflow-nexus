  ...
  return (
    <header className="sticky top-0 z-40 bg-white/95 dark:bg-[#16261C]/95 backdrop-blur-md border-b border-[#D4EEDC] dark:border-[#253D2C] shadow-xs transition-colors duration-200">
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between min-h-[68px] py-2 gap-3 md:gap-6">
          
          {/* Left: Brand Logo & Active Business Project */}
            ...
            {currentUser.role === 'ENTREPRENEUR' && activeProject && (
              <div className="relative hidden xl:block ml-2">
                <button
                  ...
                  className="flex items-center gap-2.5 px-4 py-2 rounded-2xl bg-[#F0FAF3] dark:bg-[#1E3326] hover:bg-[#DCF5E4] dark:hover:bg-[#253D2C] border border-[#D4EEDC] dark:border-[#2A4736] transition-all text-xs text-[#192A1E] dark:text-[#E8F7ED] font-medium cursor-pointer shadow-xs"
                >
                  <Building2 className="w-4 h-4 text-[#2E6F40] dark:text-[#68BA7F] shrink-0" />
                  <div className="text-left">
                    <div className="text-[10px] text-[#60826A] dark:text-[#A3D4B3] font-semibold uppercase tracking-wider">Active Business:</div>
                    <div className="font-extrabold text-[#192A1E] dark:text-white max-w-[200px] truncate">{activeProject.businessName}</div>
                  </div>
                  <ChevronDown className="w-3.5 h-3.5 text-[#60826A] dark:text-[#A3D4B3] ml-1" />
                </button>

                {projectDropdownOpen && (
                  <div className="absolute left-0 mt-2 w-80 bg-white dark:bg-[#1B2D23] rounded-2xl shadow-xl border border-[#D4EEDC] dark:border-[#2A4736] py-2 z-50 animate-in fade-in duration-150">
                    <div className="px-4 py-1.5 text-[11px] font-extrabold text-[#60826A] dark:text-[#A3D4B3] uppercase tracking-wider">
                    ...
                    {projects.map(p => (
                      <button
                        ...
                        className={`w-full text-left px-4 py-2.5 text-xs flex items-center justify-between hover:bg-[#F0FAF3] dark:hover:bg-[#253D2C] transition-colors ${
                          p.id === activeProjectId ? 'bg-[#CFFFDC]/40 dark:bg-[#253D2C] text-[#2E6F40] dark:text-[#CFFFDC] font-bold' : 'text-[#253D2C] dark:text-[#D1E8DA]'
                        }`}
                      >
                        <div className="truncate">
                          <div className="font-bold truncate">{p.businessName}</div>
                    ...
                    <div className="border-t border-[#D4EEDC] dark:border-[#2A4736] mt-1 pt-1.5 px-3">
                      <button
                        onClick={() => {
                          setActiveTab('new-project');
                          setProjectDropdownOpen(false);
                        }}
                        className="w-full flex items-center justify-center gap-1.5 px-3 py-2 text-xs text-[#2E6F40] dark:text-[#CFFFDC] hover:bg-[#F0FAF3] dark:hover:bg-[#253D2C] rounded-xl font-extrabold transition-colors cursor-pointer"
                      >
          ...
          {/* Right Controls Group (Generously Spaced) */}
          <div className="flex items-center gap-3 sm:gap-4 lg:gap-5 shrink-0">
            ...
            {/* Role Switcher Pills */}
            <div className="hidden lg:flex items-center gap-1.5 bg-[#F0FAF3] dark:bg-[#1E3326] p-1.5 rounded-xl border border-[#D4EEDC] dark:border-[#2A4736] text-[11px]">
              <button
                onClick={() => switchRole('ENTREPRENEUR')}
                className={`px-3 py-1 rounded-lg font-extrabold transition-all cursor-pointer ${currentUser.role === 'ENTREPRENEUR' ? 'bg-[#2E6F40] text-white shadow-xs' : 'text-[#4A6B53] dark:text-[#A3D4B3] hover:text-[#253D2C] dark:hover:text-white'}`}
              >
                Entrepreneur
              </button>
              <button
              ...
              <button
                ...
                className={`px-3 py-1 rounded-lg font-extrabold transition-all cursor-pointer ${currentUser.role === 'ADMIN' ? 'bg-[#2E6F40] text-white shadow-xs' : 'text-[#4A6B53] dark:text-[#A3D4B3] hover:text-[#253D2C] dark:hover:text-white'}`}
              >
                Admin
              </button>
            </div>
            ...
            <div className="relative pl-1">
              <button
                ...
                className="flex items-center gap-2.5 p-1.5 rounded-2xl hover:bg-[#F0FAF3] dark:hover:bg-[#1E3326] transition-all border border-[#D4EEDC] dark:border-[#2A4736] cursor-pointer shadow-xs"
              >
                <div className="w-8 h-8 rounded-xl bg-[#253D2C] dark:bg-[#2E6F40] flex items-center justify-center text-[#CFFFDC] font-extrabold text-xs shrink-0">
